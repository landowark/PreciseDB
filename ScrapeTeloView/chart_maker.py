...
def getTreatments(doc):
    ...
    for item in treatments.keys():
        try:
            ...
            dicto['end'] = mdates.datestr2num(doc['treatments'][item]['End Date'])
            dicto['name'] = doc['treatments'][item]['Treatment']
...
def vsPSA(patientnumber="MB0389PR", parametername='meanInt', figPath="C:\\Users\\Landon\\Desktop\\"):
    ...
    data = list(zip([mdates.datestr2num(doc['filters'][filter]['DateRec']) for filter in filters], [doc['filters'][filter][parametername] for filter in filters]))
    ...
    ax1.plot([x[0] for x in psa], [y[1] for y in psa], 'r', marker='.')
